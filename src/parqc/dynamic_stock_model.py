# -*- coding: utf-8 -*-
"""
The work here is based on ODYM and RBSM.
...
def __version__():
    ...
    # FIXME : based on ODYM v1.0 and RBSM
    """
    return str("1.0"), str(
...
class DynamicStockModel(object):
    ...
    cohort c
    type j
    renovations r (not yet implemented)
    ...
    def __init__(
        ...
        self.removed_cg = removed_cg  # optional
    ...
    def check_stock_balance(self):
        ...
        try:
            ...
            return Balance
        except:  # FIXME avoid bare except clause
    ...
    def compute_sf(self):  # survival functions
        ...
        if self.sf is None:
            ...
            if (
                self.lt["Type"] == "LogNormal2"
            ):  # lognormal distribution to use when mean and stddev of lognormal curve are directly know from survreg; to use with the values from Table 4 of this paper : https://ascelibrary-org.acces.bibl.ulaval.ca/doi/full/10.1061/%28ASCE%29AE.1943-5568.0000401#appendix. Should not be necessry, but I don't quite get the previous implementation... # FIXME
                for m in range(0, len(self.t)):  # cohort index
                    if (
                        self.lt["Mean"][m] != 0
                    ):  # For products with lifetime of 0, sf == 0
    ...
    def compute_stock_driven_model(self, NegativeInflowCorrect=False):
        ...
        if self.s is not None:
            if self.lt is not None:
                # NOTE/FIXME: new addition, add to other models?
                # Track per-year correction flag (already present) and per-cell removals
                removed_cg = np.zeros((len(self.t), len(self.t)))  # year × cohort
                ...
                for m in range(
                    ...
                    if (
                        ...
                        if (
                            ...
                            # NOTE/FIXME: new addition, add to other models?
                            # Amount removed from each cohort/type at year m:
                            removed_cg[m, :] = (self.s_c[m, :] * Delta_percent).copy()
                ...
                self.removed_cg = removed_cg  # expose the full per-year, per-cohort, per-type removals
    ...
    # def TODO: compute stock driven model w/ typesplit
    ...
    def compute_stock_driven_model_initialstock_typesplit_negativeinflowcorrect(
        ...
        if self.s is not None:
            if self.lt is not None:
                ...
                # NOTE/FIXME: new addition, add to other models?
                # Track per-year correction flag (already present) and per-cell removals
                removed_cg = np.zeros((Ntt, Ntt, Ng))     # amount removed in correction year m, by cohort c and type 
                ...
                if NegativeInflowCorrect is True:
                    if NegativeInflowStyle == 'Aging':  # TODO validate; extend to two other models? 
                        # Aging style: let the stock age naturally without removing extra items.
                        # If negative inflow would occur, set inflow to 0 but don't adjust existing cohorts.
                        # WARNING: This will lead to unbalanced stocks (actual stock > prescribed stock)
                        # when negative inflows occur. Use for sensitivity analysis only.
                        ...
                        for m in range(
                            ...
                            if i0_test < 0:
                                # Negative inflow detected - set inflow to zero and let stock age naturally
                                NIC_Flags[m] = i0_test
                                i_g[m, :] = 0  # Set inflow to 0, but don't redistribute gap
                                
                                # if not warned_once:
                                #     logger.warning(
                                #         f"NegativeInflowStyle='Aging': Negative inflow detected at year index {m}. "
                                #         "Inflow set to 0, but stock will NOT be corrected. "
                                #         "This leads to unbalanced stocks (actual > prescribed). "
                                #         "Use for sensitivity analysis only."
                                #     )
                                #     warned_once = True
                            else:
                                # Normal case: positive inflow - compute inflows by type
                                for g in range(0, Ng):
                                    if SFArrayCombined[m, m, g] != 0:  # Else, inflow is 0.
                                        i_g[m, g] = (
                                            TypeSplit[m, g]
                            ...
                            # Compute stock and outflows for cohort m using standard formulas
                            # (if i_g[m, :] = 0, these will naturally be 0)
                            for g in range(0, Ng):
                                s_cg[m::, m, g] = i_g[m, g] * SFArrayCombined[m::, m, g]
                                o_cg[m, m, g] = i_g[m, g] * (1 - SFArrayCombined[m, m, g])
                                o_cg[m + 1::, m, g] = s_cg[m:-1, m, g] - s_cg[m + 1::, m, g]
                                
                    elif NegativeInflowStyle == 'Default': # Default behaviour
                        for m in range(
                            SwitchTime, len(self.t)
                        ):  # for all years m, starting at SwitchTime
                            # 1) Determine inflow from mass balance:
                            ...
                            if i0_test < 0:
                                ...
                                # NOTE/FIXME: new addition, add to other models?
                                # Amount removed from each cohort/type at year m:
                                removed_cg[m, :, :] = (s_cg[m, :, :] * Delta_percent).copy()
                        ...
                        raise ValueError(f"Unexpected NegativeInflowStyle value: {NegativeInflowStyle}")
